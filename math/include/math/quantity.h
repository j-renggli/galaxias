...
struct Quantity
{
    ...
    bool operator<(double value) const { return val_ < value; }
    bool operator<=(const Quantity& rhs) const { return val_ <= rhs.val_; }
    bool operator<=(double value) const { return val_ <= value; }
    bool operator==(const Quantity& rhs) const { return val_ == rhs.val_; }
    bool operator==(double value) const { return val_ == value; }
    ...
    template <class U2, class Out = typename unit::DivideUnit<U, U2>::value_type>
    Quantity<Out> operator/(const Quantity<U2>& rhs) const
    {
        return Quantity<Out>(val_ / rhs.value());
    }
    ...
    {
        return Quantity<Out>(std::pow(val_, static_cast<double>(N) / static_cast<double>(D)));
    }
